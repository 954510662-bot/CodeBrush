...
const fontFamilies = [
  ...
  'Verdana', 'Trebuchet MS', 'Courier New', 'Impact'
]

function PropertiesPanel() {
  const { projects, currentProjectId, updateLayer, updateTextStyle } = useStore()
  ...
  const handleTextColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateTextStyle(currentProjectId, selectedLayer.id, { color: { r, g, b, a: 1 } })
  }
    ...
    <div className="h-full flex flex-col overflow-y-auto">
      ...
      <div className="flex-1 p-4 space-y-6">
        ...
        {selectedLayer.type === 'text' ? (
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2">文本样式</label>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1">字体</label>
                <select
                  value={(selectedLayer.metadata.fontFamily as string) || 'Inter'}
                  onChange={(e) => updateTextStyle(currentProjectId!, selectedLayer.id, { fontFamily: e.target.value })}
              ...
              <div>
                ...
                <input
                  type="number"
                  value={(selectedLayer.metadata.fontSize as number) || 16}
                  onChange={(e) => updateTextStyle(currentProjectId!, selectedLayer.id, { fontSize: parseFloat(e.target.value) || 16 })}
                  className="w-full px-2 py-1 border border-gray-300 rounded text-xs"
                />
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={currentFill.color 
                    ? `#${currentFill.color.r.toString(16).padStart(2, '0')}${currentFill.color.g.toString(16).padStart(2, '0')}${currentFill.color.b.toString(16).padStart(2, '0')}`
                    : '#000000'}
                  onChange={handleTextColorChange}
                  className="w-10 h-10 rounded border border-gray-300 cursor-pointer"
                />
                <div className="flex-1">
                  <div className="text-xs text-gray-500">文字颜色</div>
                </div>
              </div>
            </div>
          </div>
